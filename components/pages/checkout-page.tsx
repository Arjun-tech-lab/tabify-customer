...
export default function CheckoutPage({ onOrderSubmit, onBack }: CheckoutPageProps) {
  ...
  const handleSubmit = () => {
    ...
    // ✅ Prepare single, aggregated order object
    const order = {
      id: orderId,
      user: "Ravi Kumar", // replace with logged-in user from context or auth
      total,
    ...
    // ✅ Emit to backend (this sends one single socket event)
    socket.emit("newOrder", order)
    console.log("📦 Order sent via socket:", order)

    // ✅ Optionally show confirmation and clear cart
    setTimeout(() => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-primary/5 to-background flex items-center justify-center p-4">
        ...
        >
          ...
          >
            <div className="w-12 h-12 rounded-full bg-primary flex items-center justify-center text-white text-2xl">
              ✓
            </div>
          </motion.div>
